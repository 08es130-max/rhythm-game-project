...
(function(){
  ...
  const isLrOwned=()=>isOwned(LR_ID)||isOwned(LR_AYUMU_ID)||isOwned(LR_KASUMI_ID)||isOwned(LR_SHIZUKU_ID)||isOwned(LR_KARIN_ID);
  ...
  const applyMode=(mode)=>{
    ...
    const lrSceneSrc=lrCharacter==='ayumu'&&isOwned(LR_AYUMU_ID)?LR_AYUMU_HOME:lrCharacter==='kasumi'&&isOwned(LR_KASUMI_ID)?LR_KASUMI_HOME:lrCharacter==='shizuku'&&isOwned(LR_SHIZUKU_ID)?LR_SHIZUKU_HOME:lrCharacter==='karin'&&isOwned(LR_KARIN_ID)?LR_KARIN_HOME:LR_HOME_SCENE;
