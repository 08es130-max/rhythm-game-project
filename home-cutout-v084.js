...
(function(){
  ...
  const LR_KARIN_ID='lr-karin-blue-garden';
  const LR_KANATA_ID='lr-kanata-lavender-garden';
  const LR_SETSUNA_ID='lr-setsuna-rose-garden';
  ...
  const LR_LANZHU_ID='lr-lanzhu-rose-palace';
  ...
  const LR_KARIN_HOME=`assets/lr/lr-karin-home.webp?v=${VERSION}-lrhome1`;
  const LR_KANATA_HOME=`assets/lr/lr-kanata-home.webp?v=${VERSION}-lrhome1`;
  const LR_SETSUNA_HOME=`assets/lr/lr-setsuna-home.webp?v=${VERSION}-lrhome1`;
  ...
  const LR_LANZHU_HOME=`assets/lr/lr-lanzhu-home.webp?v=${VERSION}-lrhome1`;
  ...
  const isLrOwned=()=>isOwned(LR_ID)||isOwned(LR_AYUMU_ID)||isOwned(LR_KASUMI_ID)||isOwned(LR_SHIZUKU_ID)||isOwned(LR_KARIN_ID)||isOwned(LR_KANATA_ID)||isOwned(LR_SETSUNA_ID)||isOwned(LR_AI_ID)||isOwned(LR_EMMA_ID)||isOwned(LR_MIA_ID)||isOwned(LR_RINA_ID)||isOwned(LR_LANZHU_ID);
  ...
  function installHomeArtSelector(){
    ...
    style.textContent=`
      ...
      .lr-home-character-option{width:150px;border:2px solid #334155;border-radius:12px;background:#0b1220;padding:9px 6px 7px;color:#f8fafc}
      .lr-home-character-option.is-selected{border-color:#38bdf8;box-shadow:0 0 0 2px rgba(56,189,248,.18)}
      .lr-home-character-color{display:block;width:22px;height:22px;margin:0 auto 5px;border-radius:50%;background:var(--lr-color,#fff);box-shadow:0 0 0 2px rgba(255,255,255,.2)}
      .lr-home-character-option strong{display:block;margin-top:2px;text-align:center;font-size:11px;white-space:nowrap;word-break:keep-all}
    ...
    if(lrBtn){
      ...
      const refreshLrChoices=()=>{
        ...
        const themeColors={
          ayumu:'#ed7d95',kasumi:'#e7d600',shizuku:'#01b7ed',karin:'#485ec6',
          ai:'#ff5800',kanata:'#a664a0',setsuna:'#d81c2f',emma:'#84c36e',
          rina:'#9ca5b9',shioriko:'#37b484',mia:'#d6d6d6',lanzhu:'#f8c8c4'
        };
        owned.forEach(ch=>{
          ...
          choice.style.setProperty('--lr-color',themeColors[ch.id]||'#fff');
          choice.innerHTML=`<span class="lr-home-character-color" aria-hidden="true"></span><strong>${ch.label}</strong>`;
