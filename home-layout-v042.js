...
(function(){
  ...
  const HISTORY=[
    {version:'0.8.89',title:'LR栞子の画像と顔位置を修正',items:['壊れていたLR専用WebPを埋め込み元データから正常な画像として復元しました。','LR演出でマンスリーUR栞子が表示される問題を解消しました。','LR結果カードはUR同様に大きな全面アート表示のまま、顔が見切れないよう上寄せに調整しました。','LR演出側も顔優先の表示位置へ調整しました。']},
    {version:'0.8.88',title:'LR栞子のカード表示を修正',items:['LR演出でマンスリーUR画像が表示される問題を修正しました。','LR結果カードを小さい丸アイコン表示から大きな全面アート表示へ変更しました。','LRカードとライブアイコンで顔が見切れないよう専用画像を調整しました。','LRホーム立ち絵の透明背景素材を正式登録しました。']},
    {version:'0.8.87',title:'最高レアリティLRを追加',items:['最高レアリティLRを新設し、三船栞子【煌めくミントローズ】を追加しました。','通常提供割合はLR 0.01%・UR 1%・N 98.99%です。','LRはURより強い専用フラッシュ・発光・虹色演出で開封されます。','LR栞子を獲得すると専用ホーム立ち絵と専用会話が解放され、ライブ用アイコンも部室で使用できます。','「URまでスキップ」はNだけを飛ばしてLR/UR演出を表示し、「全スキップ」はLRを含めて結果一覧へ進みます。','管理人の隠し部屋に「先頭1枠LR・残りUR100%」の演出確認モードを追加しました。']},
    {version:'0.8.86',title:'ガチャのテンポとスキップを改善',items:['Nは短い間隔で連続開封し、URだけ特別演出で止まるようにしました。','「URまでスキップ」はNだけ飛ばし、UR演出は表示します。','「全スキップ」はURを含めてすべて開封し、結果一覧へ進みます。','100連勧誘を追加し、Nは自動スキップ、URのみ演出します。','100連の結果は小型カードのスクロール一覧で表示し、画像は遅延読み込みにしました。','ガチャ画面の連打によるダブルタップ拡大を抑制しました。']},
