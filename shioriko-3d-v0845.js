// Ver.0.8.45: lightweight interactive Shioriko-inspired 3D prototype for the interaction room.
(function(){
  'use strict';
  ...
  async function init(slot,message){
    ...
    // hair cap + long back hair + side locks
    ...
    canvas.addEventListener('pointermove',e=>{if(!pointerDown)return;const dx=e.clientX-startX,dy=e.clientY-startY;if(Math.hypot(dx,dy)>5)moved=true;targetYaw=startYaw+dx*.010;targetPitch=Math.max(-.18,Math.min(.20,startPitch+dy*.004));});
    ...
    message.textContent='3D栞子（プロトタイプ）を読み込みました。ドラッグで回転、頭や手をタップしてみてください。';
    document.querySelector('.interaction-touch-hint')?.replaceChildren(document.createTextNode('ドラッグで回転／頭・手をタップ'));
